...
export default function FieldItemBox(props: props) {
    ...
    return (
        <Grid columns={10} container className={'relative bg-zinc-100 bg-opacity-50 dark:bg-opacity-30 dark:bg-black w-full text-black dark:text-slate-300 rounded-2xl py-3 px-4 md:px-6 shadow-lg hover:shadow-2xl border dark:hover:shadow-emerald-500/[0.1] dark:border-white/[0.2] border-black/[0.1] ' + className}>
            ...
            <Grid size={10}><div className='mt-1 h-full flex items-center justify-between'><Link className='dark:text-zinc-200' to={type}>browse more {type}</Link><button className="px-4 py-2 rounded-xl bg-black dark:bg-white dark:text-black text-white text-xs font-bold" >Details</button></div></Grid>
        </Grid>
    )
}
